...
- main에 푸시하면 일곱 가지 검사 후 dist/만 배포합니다.
- 공개 자료는 슬라이드·커리큘럼·디자인 시스템이며 스크립트·발표자 노트는 제외합니다.

## 파일과 원본 관리

- dist/index.html: 모든 챕터를 기본으로 접어 두는 자료실. 챕터 제목을 누르면 펼쳐지며, 강의별 슬라이드 열기와 수강 완료 체크를 제공합니다.
- dist/materials/: 대본·녹화 지시·스크립트 URL을 제외한 공개 HTML·JS와 실습용 Markdown.
...
섹션 00·01·02의 16개 강의 모두 dist/assets/course-player.js와 course-player.css를 사용합니다. 이 공통 플레이어가 홈·이전/다음·장 번호·전체화면·패널 숨김, 키보드·터치 이동, 주소 해시, 화면 크기 조정, 진행 표시와 접근성 상태를 관리합니다. 개별 HTML에는 조작 패널이나 이벤트 코드를 복사하지 않습니다. 기존 HTML형 0-1과 데이터형 강의는 같은 플레이어에 본문만 제공합니다.
...
v0.4 기준 2-1~2-5, 5강·50장·45분입니다. 필요한 컨텍스트 선택, 저장소 규칙, 제품·설계·작업 문서 분리, 역할별 책임, 작은 diff와 승인 게이트를 다룹니다.

[컨텍스트 실습지와 네 문서 예시](dist/materials/section02-context-workbook.md)에 AGENTS.md, docs/PRODUCT.md, docs/ARCHITECTURE.md, docs/TASKS.md를 제공합니다. 별도의 SignalDesk 실습 저장소에 맞춰 작성하며 이 강의 사이트의 지침을 덮어쓰지 않습니다. 새 세션의 읽기 전용 요약과 원본 문서를 대조하는 것으로 미션 2를 확인합니다.

가상 응답·합성 공지·예상 결과는 실제 AI 실험이나 앱 구현·테스트 통과 기록이 아닙니다. T-001의 함수 반환값 검증과 AC-02의 화면 상태 검증을 구분합니다. 제목·시간·문서 연결과 공통 플레이어는 자동 검사하며, 이번 추가 자료의 실제 브라우저 화면 검수는 수행하지 않았습니다.
...
- node tests/validate-section02.cjs
